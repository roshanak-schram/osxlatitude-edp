<?php
	...
	if ($action == "") { echo "No action defined.."; exit; }
	if ($action == "dellBiosCrack") { runDellBiosCrack(); exit; }
	
	if ($action == "dellBiosCrack") 		{ runDellBiosCrack(); exit; }
	if ($action == "install-mc") 			{ echo "<pre>"; downloadAndRun("http://www.osxlatitude.com/files/mc.dmg", "dmg", "mc.dmg", "/Volumes/mc.pkg/mc.pkg"); exit; }
	if ($action == "install-htop") 			{ echo "<pre>"; downloadAndRun("http://www.osxlatitude.com/files/htop.pkg", "pkg", "htop.pkg", "/downloads/htop.pkg"); exit; }
	if ($action == "install-lynx") 			{ echo "<pre>"; downloadAndRun("http://www.osxlatitude.com/files/lynx.dmg", "dmg", "lynx.dmg", "/Volumes/Lynx-2.8.7d9-10.5.1+u/install.command"); exit; }
	if ($action == "install-istat2") 		{ echo "<pre>"; downloadAndRun("http://www.osxlatitude.com/files/istat2.dmg", "dmg", "istat2.dmg", "/Volumes/istat2/install.app"); exit; }
	
	if ($action == "fix-touch-sle") 		{ echo "<pre>"; fixes_touch_sle(); exit; }
	if ($action == "toogle-hibernation") 	{ echo "<pre>"; fixes_toogleHibernationMode(); exit; }
	if ($action == "console-slow-start") 	{ echo "<pre>"; fixes_slowconsole(); exit; }
	if ($action == "fix-sound-delay") 		{ echo "<pre>"; fixes_soundelay(); exit; }

	if ($action == "fix-biometric") 		{ echo "<pre>"; fixes_biometric(); exit; }
	if ($action == "fix-sound-delay") 		{ echo "<pre>"; fixes_soundelay(); exit; }
	if ($action == "fix-console-colors") 	{ echo "<pre>"; fixes_console_colors(); exit; }
	...
	function fixes_toogleHibernationMode() {
		...
		if ($hibernatemode == "3") {
			...
			if (is_file("/var/vm/sleepimage")) { echo "Hibernation file was found... removing.. <br>"; system("rm -Rf /var/vm/sleepimage"); }
			echo "Fix applied..<br>"; 			
		}
		if ($hibernatemode == "0") {
